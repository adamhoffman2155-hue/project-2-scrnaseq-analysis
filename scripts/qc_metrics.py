...
def plot_qc_metrics(adata, save_path=None):
    ...
    # nGenes vs nCounts
    axes[0, 0].scatter(adata.obs["total_counts"], adata.obs["n_genes_by_counts"], alpha=0.5, s=10)
...
def filter_cells(adata, min_genes=200, max_genes=5000, max_mt=20, max_rb=50):
    ...
    print(
        f"Filtered {initial_n - adata.n_obs} cells ({100 * (initial_n - adata.n_obs) / initial_n:.1f}%)"
    )
...
def filter_genes(adata, min_cells=3, min_counts=3):
    ...
    expressed = (
        np.sum(adata.X > 0, axis=0).A1 if hasattr(adata.X, "A1") else np.sum(adata.X > 0, axis=0)
    )
    ...
    print(
        f"Filtered {initial_n - adata.n_vars} genes ({100 * (initial_n - adata.n_vars) / initial_n:.1f}%)"
    )
